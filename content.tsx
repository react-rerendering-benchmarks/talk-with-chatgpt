...
function initListen({
  ...
  function startListen() {
    ...
    recognition.onresult = event => {
      ...
      const StopAnswerWordIndex = text.replace(/\s/g, '').toLowerCase().indexOf(stopAnswerWord.replace(/\s/g, '').toLowerCase());
  ...
  if (currentMessageDom) {
    ...
    if (text !== prevText) {
      ...
      for (let i = currentMessageStep; i < sentences.length; i++) {
        ...
        if (currentMessageStep + 1 === sentences.length && sentences[sentences.length - 1].trim() !== '') {
          return;
        }
        if (sentences[i].trim()) {
...
function toStopAnswer(setCurrentPlayerStatus) {
  toStopAnswerIdCount++;
  const button = document.querySelector('.stretch').querySelectorAll('button')[0];
...
const IndexContent = memo(function IndexContent() {
  const [currentPlayerStatus, setCurrentPlayerStatus] = useStorage<number>('currentPlayerStatus', playerStatus.BeforeListenVoice);
  useEffect(() => {
  ...
  const [recognitionStopWord, setRecognitionStopWord] = useStorage('recognitionStopWord', 'stop');
  const [stopAnswerWord, setStopAnswerWord] = useStorage('stopAnswerWord', 'stop answer');
  ...
  const isUseAnimating = useMemo<boolean>(() => ![playerStatus.BeforeListenVoice,
  // playerStatus.SendChatMessage,
  playerStatus.SpeechPause, playerStatus.SpeechStop, playerStatus.Speechend].includes(currentPlayerStatus), [currentPlayerStatus]);
  return <div className={style.talkContainer}>
      <img width={'100%'} className={[style.recordPlayer, isUseAnimating ? style.playerPlaying : ''].join(' ')} src={recordPlayerBg} alt="" />
       <div className={[style.innerContainer].join(' ')} onClick={() => toggleStatus(false)}>
        <img className={style.innerPlayerBg} src={bg ? bg : iconBg} alt="" />
        <StatusComponentIcon className={isUseAnimating ? style.breathBg : ''} currentPlayerStatus={currentPlayerStatus}></StatusComponentIcon>
      </div>
      <img className={[style.playerController, isUseAnimating ? style.controllerOnPlayer : ''].join(' ')} onClick={() => toggleStatus(true)} src={playerControllerBg} alt="" />
     
      <SettingsIcon onClick={() => popupDrawer()} className={`${style.settingsButton} ${isUseAnimating ? style.playerPlayingReverse : ''}`} />
      <Drawer anchor="right" open={showDrawer} onClose={() => setShowDrawer(false)}>
        <MenuList userLang={userLang} setUserLang={setUserLang} rate={rate} setRate={setRate} volume={volume} setVolume={setVolume} pitch={pitch} setPitch={setPitch} answerLang={answerLang} setAnswerLang={setAnswerLang} recognitionStopWord={recognitionStopWord} setRecognitionStopWord={setRecognitionStopWord} stopAnswerWord={stopAnswerWord} setStopAnswerWord={setStopAnswerWord}></MenuList>
      </Drawer>
    </div>;
});
export default IndexContent;
function StatusComponentIcon({
  currentPlayerStatus,
  className
}) {
  const classNames = [style.innerPlayerBg, className].join(' ');
  return currentPlayerStatus === playerStatus.BeforeListenVoice ? <KeyboardVoiceIcon className={classNames} style={{
    backgroundColor: 'rgba(135, 206, 235,0.2)'
  }} /> : currentPlayerStatus === playerStatus.ListenVoicing ? <KeyboardVoiceIcon className={classNames} /> : currentPlayerStatus === playerStatus.SendChatMessage ? <TelegramIcon className={classNames} /> : currentPlayerStatus === playerStatus.Speechling ? <HeadsetIcon className={classNames} /> : currentPlayerStatus === playerStatus.SpeechPause ? prevPlayerStatus > 2 ? <HeadsetIcon className={classNames} style={{
    backgroundColor: 'rgba(135, 206, 235,0.2)'
  }} /> : <KeyboardVoiceIcon className={classNames} style={{
    backgroundColor: 'rgba(135, 206, 235,0.2)'
  }} /> : <div> </div>;
}